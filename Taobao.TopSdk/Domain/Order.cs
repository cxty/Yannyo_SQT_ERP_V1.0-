...
namespace Top.Api.Domain
{
    ...
    public class Order : TopObject
    {
        ...
        /// <summary>
        /// 买家实付金额（单笔子订单时包含物流费用，多笔子订单时不包含物流费用 ，付款后这个价格不会变化）。精确到2位小数，单位:元。如:200.07，表示:200元7分。规则为：total_fee*num - audjust_fee - discount_fee - refund_fee
        /// </summary>
        [XmlElement("payment")]
        ...
        /// <summary>
        /// 订单状态（请关注此状态，如果为TRADE_CLOSED_BY_TAOBAO状态，则不要对此订单进行发货，切记啊！）。可选值:  <ul> <li>TRADE_NO_CREATE_PAY(没有创建支付宝交易)  <li>WAIT_BUYER_PAY(等待买家付款)  <li>WAIT_SELLER_SEND_GOODS(等待卖家发货,即:买家已付款)  <li>WAIT_BUYER_CONFIRM_GOODS(等待买家确认收货,即:卖家已发货)  <li>TRADE_BUYER_SIGNED(买家已签收,货到付款专用)  <li>TRADE_FINISHED(交易成功)  <li>TRADE_CLOSED(付款以后用户退款成功，交易自动关闭)  <li>TRADE_CLOSED_BY_TAOBAO(付款以前，卖家或买家主动关闭交易)  <li>ALL_WAIT_PAY(包含：WAIT_BUYER_PAY、TRADE_NO_CREATE_PAY)  <li>ALL_CLOSED(包含：TRADE_CLOSED、TRADE_CLOSED_BY_TAOBAO) </ul>
        /// </summary>
        [XmlElement("status")]
