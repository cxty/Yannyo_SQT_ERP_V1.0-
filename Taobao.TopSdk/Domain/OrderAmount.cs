...
namespace Top.Api.Domain
{
    ...
    public class OrderAmount : TopObject
    {
        ...
        /// <summary>
        /// 子订单实付金额。精确到2位小数，单位:元。如:200.07，表示:200元7分。计算公式如下：payment = price * num + adjust_fee - discount_fee + post_fee(邮费，单笔子订单时子订单实付金额包含邮费，多笔子订单时不包含邮费)；对于退款成功的子订单，如果其所属的主订单只有一笔交易，则payment的值按上面公式计算；如果其所属的主订单有多笔交易，则该子订单的payment值为0.00
        /// </summary>
        [XmlElement("payment")]
