...
namespace Yannyo.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public int AddStoresInfo(StoresInfo model)
        {
            ...
            strSql.Append("sName,sCode,sType,RegionID,sState,sAppendTime,YHsysID,sIsFZYH,sDoDay,sDoDayMoney,PaymentSystemID,CustomersClassID,PriceClassID,sContact,sTel,sAddress,sEmail,sLicense)");
            ...
            parameters[17].Value = model.sLicense;
        ...
        public void UpdateStoresInfo(StoresInfo model)
        {
            ...
            parameters[18].Value = model.sLicense;
        ...
        /// <returns></returns>
        public DataTable getSalesExportData(DateTime bDate, int dType)
        {
            SqlParameter[] parameters = {
                    new SqlParameter("@dtmbDate", SqlDbType.DateTime, 8),
                    new SqlParameter("@inyType", SqlDbType.Int, 4),
                                             };

            parameters[0].Value = bDate;
            parameters[1].Value = dType;

            return DbHelper.ExecuteDataset(CommandType.StoredProcedure, "sp_" + BaseConfigs.GetTablePrefix + "getCompanySales_Details", parameters).Tables[0];
		...
		public bool CheckStoresByOrder(int StoresID){
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select count(1) from tbOrderInfo where StoresID=@StoresID and oSteps<>1");
