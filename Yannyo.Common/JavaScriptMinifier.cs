...
namespace Yannyo.Common
{
    public class JavaScriptMinifier
    {
        ...
        */
        public void Minify(string src, out string dst)
        {
            byte[] bs2 = System.Text.Encoding.UTF8.GetBytes(src);
            sr = new MemoryStream(bs2);
            try
            {
                jsmin();
                dst = GetStreamString(sw);
            }
            finally
            {
                sw.Close();
        ...
        */
        ...
        public string GetStreamString(System.IO.MemoryStream ms)
        {
