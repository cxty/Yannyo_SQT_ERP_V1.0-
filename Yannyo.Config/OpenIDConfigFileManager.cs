...
{
    class OpenIDConfigFileManager : Yannyo.Config.DefaultConfigFileManager
    {
        ...
        /// 初始化文件修改时间和对象实例
        /// </summary>
        static OpenIDConfigFileManager()
        {
            m_fileoldchange = System.IO.File.GetLastWriteTime(ConfigFilePath);

            try
            {
                m_configinfo = (OpenIDConfigInfo)DefaultConfigFileManager.DeserializeInfo(ConfigFilePath, typeof(OpenIDConfigInfo));
            }
            catch
            {
        ...
        private static void ReviseConfig()
        {
            ...
            //1.0正式版升级到2.0
            if (doc.DocumentElement.Name != typeof(OpenIDConfigInfo).Name)
            {
                XmlDocument newdoc = new XmlDocument();
                XmlNode declarenode = newdoc.CreateXmlDeclaration("1.0", null, null);

                newdoc.AppendChild(declarenode);
                XmlNode rootnode = newdoc.CreateElement(typeof(OpenIDConfigInfo).Name);
        ...
        public new static IConfigInfo ConfigInfo
        {
            ...
            set { m_configinfo = (OpenIDConfigInfo)value; }
        }
        ...
        public new static string ConfigFilePath
        {
            get
            {
                if (filename == null)
                {
                    filename = Utils.GetMapPath(BaseConfigs.GetSysPath + "/config/openid.config");
                }
        ...
        /// </summary>
        /// <returns></returns>
        public static OpenIDConfigInfo LoadConfig()
        {
            ...
            return ConfigInfo as OpenIDConfigInfo;
        }
