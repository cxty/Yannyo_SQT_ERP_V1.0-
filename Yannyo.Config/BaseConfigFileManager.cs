...
namespace Yannyo.Config
{
    ...
    /// 基本设置信息管理类
    /// </summary>
    class BaseConfigFileManager : Yannyo.Config.DefaultConfigFileManager
    {
        ...
        public new static string ConfigFilePath
        {
            get
            {
                if (filename == null)
                {
                    ...
                    if (context != null)
                    {
                        filename = context.Server.MapPath("~/Yannyo.config");
                    ...
                    {
                        filename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Yannyo.config");
                    }

                    if (!File.Exists(filename))
                    {
                        throw new HttpException("发生错误: 虚拟目录或网站根目录下没有正确的Yannyo.config文件");
                    }
                }
                return filename;
            }
