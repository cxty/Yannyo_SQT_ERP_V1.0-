...
CREATE TABLE [dbo].[tbStoreStorehouseInfo](
	...
	[pAppendTime] [datetime] NOT NULL,
	[pUpdateTime] [datetime] NOT NULL,
 CONSTRAINT [PK__tbStoreStorehous__567ED357] PRIMARY KEY CLUSTERED 
(
	[productStorageID] ASC
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
) ON [PRIMARY]
...
CREATE TABLE [dbo].[tbStocktakeInfo](
	...
	[StaffID] [int] NOT NULL,
	[sSteps] [int] NOT NULL,
 CONSTRAINT [PK__tbStocktakeInfo__542C7691] PRIMARY KEY CLUSTERED 
(
	[StocktakeID] ASC
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
) ON [PRIMARY]
...
GO
/****** Object:  StoredProcedure [dbo].[tbsetlastexecutescheduledeventdatetime]    Script Date: 11/15/2011 11:35:01 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
BEGIN
    ...
    begin
		while @i<@DateCount
		begin
		   insert into #tbAccountDetails_tmp(Cmonth,Cday,pzCode,CertificateID,cdName,cdMoney,cdMoneyB,cDirection,FeesSubjectID,cClassName,cCode)
		   select convert(varchar,datepart(month,qq.cDateTime)) as Cmonth
			,convert(varchar,datepart(DAY,qq.cDateTime)) as Cday,qq.cNumber as pzCode,qq.CertificateID,qq.cdName,
			  qq.cdMoney,qq.cdMoneyB,fsi.cDirection,qq.FeesSubjectID,fsi.cClassName,fsi.cCode from (
			   select cfi.CertificateID,ci.FeesSubjectID,ci.cdName,cfi.cNumber,isnull(ci.cdMoney,0) as cdMoney,isnull(ci.cdMoneyB,0) as cdMoneyB,
	...
	begin
		...
		select 
		   ...
		   pzCode,cdName,cdMoney,-cdMoney as cdMoneyA,cdMoneyB,-cdMoneyB as cdMoneyBB,cDirection,FeesSubjectID,cClassName,cCode from #tbAccountDetails_tmp 
...
WHILE @@FETCH_STATUS = 0
	begin
			--仓库表
			declare ocur2 cursor for select StorageID from tbStorageInfo 
			open ocur2
			...
			WHILE @@FETCH_STATUS = 0
				...
				select @Quantity=vStorage.Quantity,
				 ...
				 (select isnull(sum(ol.oQuantity),0) as Quantity from tbOrderListInfo as ol where oWorkType=1  and ol.OrderID in(select oll.OrderID from tbOrderInfo as oll where oll.oState=0 and oll.oType in(1,4,8,9) and oll.oSteps>=4) and ol.StorageID=@StorageID and ol.ProductsID=@ProductsID and ol.oAppendTime<=@dT) as vStorageIn,--ol.oAppendTime<=@dT
				 (select isnull(sum(ool.oQuantity),0) as Quantity from tbOrderListInfo as ool where oWorkType=1 and ool.OrderID in(select ooll.OrderID from tbOrderInfo as ooll where ooll.oState=0 and ooll.oType in(2,3,5,6,7,10,11)) and ool.StorageID=@StorageID and ool.ProductsID=@ProductsID and ool.oAppendTime<=@dT) as vStorageOut ;--ool.oAppendTime<=@dT
...
-- 匹配门店产品单价
-- =============================================
CREATE PROCEDURE [dbo].[sp_StorehouseStoragePriceSynInfo]
...
BEGIN
	...
	set @error=0;
	BEGIN TRAN --申明事务
	declare scur cursor for select ProductsID from tbStoreStorehouseProductsPriceInfo where ProductsID=0
    open scur 
		--开始循环游标变量
		FETCH NEXT FROM scur INTO @temp
		WHILE @@FETCH_STATUS = 0 
		begin  
			update tbStoreStorehouseProductsPriceInfo set tbStoreStorehouseProductsPriceInfo.ProductsID=p.ProductsID 
				from tbProductsInfo as p  
				where p.pName=tbStoreStorehouseProductsPriceInfo.ProductsName or p.pBarcode=tbStoreStorehouseProductsPriceInfo.ProductsBarcode
			set @counts=@counts+1
			set @error=@error+@@error
			FETCH NEXT FROM scur INTO @temp
		end
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[sp_CostDetails_Montn]
    @dtmbDate datetime,  -- 一月至六月
    @dtmeDate datetime
AS
BEGIN
	

	select si.sName,isnull(isnull(zp_1.zping,0)+isnull(zp_2.zping,0),0) as zengping,isnull(pp.cdMoney,0) as fanli,isnull(qq.cdMoney,0) as chenlie,(isnull(dq.toObjectID,0)+isnull(ht.toObjectID,0)+isnull(jc.toObjectID,0)+isnull(tm.toObjectID,0)+isnull(gl.toObjectID,0)) as shangchang
	,isnull(mm.cdMoney,0) as yewu from tbStoresInfo as si left join(

	--赠品1
	select oi.StoresID,isnull(sum(isnull(oli.oQuantity,0)*dbo.fun_GetProductPriceXP(oli.ProductsID,@dtmbDate,@dtmeDate)),0) as zping from tbOrderInfo as oi left join tbOrderListInfo  as oli on oi.OrderID=oli.OrderID
	 where oi.oType=3 and oi.oState=0 and oli.oWorkType=2 and IsGifts<>0 and oi.OrderID in
	 (select OrderID from tbOrderWorkingLogInfo where oType=5 and DATEDIFF(MONTH,pAppendTime,@dtmbDate)=0) group by  oi.StoresID) as zp_1 on zp_1.StoresID=si.StoresID left join(
	--赠品2
	 select oi.StoresID,isnull(sum(isnull(oli.oQuantity,0)*dbo.fun_GetProductPriceXP(oli.ProductsID,@dtmbDate,@dtmeDate)),0) as zping from tbOrderInfo as oi left join tbOrderListInfo  as oli on oi.OrderID=oli.OrderID
	 where oi.oType=5 and oi.oState=0 and oli.oWorkType=2 and oi.OrderID in
	 (select OrderID from tbOrderWorkingLogInfo where oType=5 and DATEDIFF(MONTH,pAppendTime,@dtmbDate)=0) group by  oi.StoresID) as zp_2 on zp_2.StoresID=si.StoresID left join (
	  
	  select toObjectID,isnull(sum(cdMoney),0) as cdMoney from tbCertificateDataInfo where toObject=0 and
	  CertificateID in(select CertificateID from tbCertificateInfo where cState=0 and 
	  DATEDIFF(MONTH,cDateTime,@dtmbDate)=0) and FeesSubjectID in 
	  (select FeesSubjectClassID from tbFeesSubjectClassInfo where cCode='404001003') group by toObjectID) as pp on si.StoresID=pp.toObjectID left join(
    
     select toObjectID,isnull(sum(cdMoney),0) as cdMoney from tbCertificateDataInfo where toObject=0 and
	  CertificateID in(select CertificateID from tbCertificateInfo where cState=0 and 
	  DATEDIFF(MONTH,cDateTime,@dtmbDate)=0) and FeesSubjectID in 
	  (select FeesSubjectClassID from tbFeesSubjectClassInfo where cCode='404001002') group by toObjectID ) as qq on qq.toObjectID=si.StoresID left join(
   
      select toObjectID,isnull(sum(cdMoney),0) as cdMoney from tbCertificateDataInfo where toObject=0 and
	  CertificateID in(select CertificateID from tbCertificateInfo where cState=0 and 
	  DATEDIFF(MONTH,cDateTime,@dtmbDate)=0) and FeesSubjectID in 
	  (select FeesSubjectClassID from tbFeesSubjectClassInfo where cCode='404002005') group by toObjectID ) as mm on mm.toObjectID=si.StoresID left join(
      
       select toObjectID,isnull(sum(cdMoney),0) as cdMoney from tbCertificateDataInfo where toObject=0 and
	  CertificateID in(select CertificateID from tbCertificateInfo where cState=0 and 
	  DATEDIFF(MONTH,cDateTime,@dtmbDate)=0) and FeesSubjectID in 
	  (select FeesSubjectClassID from tbFeesSubjectClassInfo where cCode='404001005') group by toObjectID ) as dq on dq.toObjectID=si.StoresID left join(
       
        select toObjectID,isnull(sum(cdMoney),0) as cdMoney from tbCertificateDataInfo where toObject=0 and
	  CertificateID in(select CertificateID from tbCertificateInfo where cState=0 and 
	  DATEDIFF(MONTH,cDateTime,@dtmbDate)=0) and FeesSubjectID in 
	  (select FeesSubjectClassID from tbFeesSubjectClassInfo where cCode='404001006') group by toObjectID ) as ht on ht.toObjectID=si.StoresID left join(
      
       select toObjectID,isnull(sum(cdMoney),0) as cdMoney from tbCertificateDataInfo where toObject=0 and
	  CertificateID in(select CertificateID from tbCertificateInfo where cState=0 and 
	  DATEDIFF(MONTH,cDateTime,@dtmbDate)=0) and FeesSubjectID in 
	  (select FeesSubjectClassID from tbFeesSubjectClassInfo where cCode='404001007') group by toObjectID ) as jc on jc.toObjectID=si.StoresID left join(
      
       select toObjectID,isnull(sum(cdMoney),0) as cdMoney from tbCertificateDataInfo where toObject=0 and
	  CertificateID in(select CertificateID from tbCertificateInfo where cState=0 and 
	  DATEDIFF(MONTH,cDateTime,@dtmbDate)=0) and FeesSubjectID in 
	  (select FeesSubjectClassID from tbFeesSubjectClassInfo where cCode='404001009') group by toObjectID ) as tm on tm.toObjectID=si.StoresID left join(
      
       select toObjectID,isnull(sum(cdMoney),0) as cdMoney from tbCertificateDataInfo where toObject=0 and
	  CertificateID in(select CertificateID from tbCertificateInfo where cState=0 and 
	  DATEDIFF(MONTH,cDateTime,@dtmbDate)=0) and FeesSubjectID in 
	  (select FeesSubjectClassID from tbFeesSubjectClassInfo where cCode='404001012') group by toObjectID ) as gl on gl.toObjectID=si.StoresID
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[sp_CostDetails_Month_Staff]
	@dtmbDate datetime,
	@dtmeDate datetime
AS
BEGIN
    select si.sName,isnull(yp.yping,0) as yangping,isnull(clv.cdMoney,0) as clv,isnull((isnull(yp.yping,0)+isnull(clv.cdMoney,0)),0) as heji from tbStaffInfo as si left join(
	--样品
	select qq.StaffID,isnull(sum(pp.oQuantity*dbo.fun_GetProductPriceXP(pp.ProductsID,@dtmbDate,@dtmeDate)),0) as yping from (
    select oi.StoresID,isnull(oli.oQuantity,0) as oQuantity,oli.ProductsID from tbOrderInfo as oi left join tbOrderListInfo as oli on oi.OrderID=oli.OrderID
	where oi.oType=6 and oi.oState=0 and oli.oWorkType=2  and oi.OrderID in (select OrderID from tbOrderWorkingLogInfo where oType=5 and 
	DATEDIFF(MONTH,pAppendTime,@dtmbDate)=0)) as pp left join (
	
	select distinct(si.StaffID),sName,ssi.StoresID from tbStaffInfo as si left join tbSales_Staff_StoresDataInfo as ssi on si.StaffID=ssi.StaffID
	where si.sState=0 and si.sType=1 and @dtmbDate between ssi.bdate and ssi.edate) as qq on pp.StoresID=qq.StoresID group by qq.StaffID) 
	as yp on yp.StaffID=si.StaffID left join(
    
	select ci.StaffID,sum(cdi.cdMoney) as cdMoney from tbCertificateDataInfo as cdi right join tbCertificateInfo as ci on cdi.CertificateID=ci.CertificateID
	where cdi.FeesSubjectID in (select FeesSubjectClassID from tbFeesSubjectClassInfo where cCode='404002001')  and ci.cState=0 and  datediff(month,ci.cDateTime,@dtmbDate)=0
	group by ci.StaffID) as clv on clv.StaffID=si.StaffID

	 where si.sType=1 and si.sState=0
END
GO
...
GO
/****** Object:  StoredProcedure [dbo].[sp_GetOutReport]    Script Date: 11/15/2011 11:35:00 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
CREATE PROCEDURE [dbo].[sp_GetSalesReport_XP]
	(
	@ReType int = 1,		--商品销售=1,客户销售=2,结算系统=3,业务员（岗位变动）=4,促销员=5,地区统计=6,业务员（单据）=7
	@bDate datetime,		--开始时间
	@eDate datetime,		--结束时间
	...
	@dType int=0,						--统计类型,0=销售统计按区间统计,1=发出商品按区间统计,2=备货商品统计(审核后发货前),3=销售统计按时间点,4=发出商品按时间点统计,5=业务员单据统计
...
begin
	...
	if @dType=0
	begin
	insert into @OrderList(OrderID,StorageID,ProductsID,oQuantity,oPrice,oMoney,StoresID,StoresSupplierID,oType,oOrderDateTime,oWorkingDateTime,IsGifts,StaffID)
	...
	begin
			insert into @OrderList(OrderID,StorageID,ProductsID,oQuantity,oPrice,oMoney,StoresID,StoresSupplierID,oType,oOrderDateTime,oWorkingDateTime,IsGifts,StaffID)
					...
					--,od.IsGifts from ( 
					--					select ol.*,o.oSteps,o.StoresID,o.oType,o.oOrderDateTime from tbOrderListInfo ol left join tbOrderInfo as o on o.OrderID=ol.OrderID where o.oType in(3,4,5,6,10) and o.oState=0 and ol.oWorkType >=1--核销后 in(1,2)
					--				 ) as od   
					...
					--		select ol.*,o.oSteps,o.StoresID,o.oType,o.oOrderDateTime from tbOrderListInfo ol left join tbOrderInfo as o on o.OrderID=ol.OrderID where o.oType in(3,4,5,6,10) and o.oState=0 and ol.oWorkType=1
					...
					--	from tbOrderListInfo ol left join tbOrderInfo as o on o.OrderID=ol.OrderID left join tbOrderWorkingLogInfo ow on o.OrderID=ow.OrderID and o.oSteps=ow.oType
	...
	begin
		...
		insert into @OrderList_tem(OrderID,StaffID,StoresID,ProductsID,oQuantity,oMoney,IsGifts,oType)
					select OrderID,StaffID,StoresID,ProductsID,oQuantity,oMoney,IsGifts,oType from @OrderList o where NOT EXISTS (select null from @OrderList_t as t where o.OrderID=t.OrderID);
	...
	begin
		...
		insert into @OrderList_tem(OrderID,StaffID,StoresID,ProductsID,oQuantity,oMoney,IsGifts,oType)
			select o.OrderID,s.StaffID,o.StoresID,o.ProductsID,o.oQuantity ,o.oMoney,o.IsGifts,o.oType 
	...
	begin
		insert into @OrderList_tem(OrderID,StaffID,StoresID,ProductsID,oQuantity,oMoney,IsGifts,oType)
					select OrderID,0,StoresID,ProductsID,oQuantity,oMoney,IsGifts,oType from @OrderList o where NOT EXISTS (select null from @OrderList_t as t where o.OrderID=t.OrderID);
	...
	insert into @OrderList_tem(OrderID,StaffID,StoresID,ProductsID,oQuantity,oMoney,IsGifts,oType)
		...
		select ps.DepartmentsClassID,ps.StaffID,ps.sName,
				...
				ss.oQuantity_y,(ss.oQuantity_y*isnull(ppp.CoPrice,0)) as oMoney_y,
				ss.oQuantity_sz,(ss.oQuantity_sz*isnull(ppp.CoPrice,0)) as oMoney_sz
		from (select * from tbStaffInfo where sType=1) as ps left join (
			select s.StaffID,s.ProductsID,
					sum(isnull(oo.oQuantity,0)) oQuantity,			--销售数量
					sum(isnull(oo.oMoney,0)) oMoney,				--销售金额
					sum(isnull(ool.oQuantity,0)) oQuantity_t,		--销售退货数量
...
set @tEDate = dateadd(DAY,365,getdate());
	...
	select ppp.* from (
			...
			on oo.ProductsID=ool.ProductsID
			left join tbProductClassInfo pc on pc.ProductClassID=p.ProductClassID) ppp where ppp.oQuantity<>0 or ppp.oMoney<>0 or ppp.oQuantity_t<>0 or ppp.oMoney_t<>0  order by  ppp.ProductClassID,ppp.ProductsID desc;
	end
	...
	begin
	select ppp.* from (
		select s.*,sc.sClassName,isnull(oo.CostPrice,0) CostPrice,isnull(oo.oMoney,0) oMoney,isnull(ool.CostPrice_t,0) CostPrice_t,isnull(ool.oMoney_t,0) oMoney_t from tbSupplierInfo as s left join(
			--进货
			select oi.StoresID,isnull(sum(ol.oQuantity*dbo.fun_GetProductCostPrice(ol.ProductsID,oi.oOrderDateTime)),0) CostPrice,isnull(sum(ol.oMoney),0) oMoney from tbOrderListInfo ol left join tbOrderInfo oi on oi.OrderID=ol.OrderID where oi.oType=1 and oi.oState=0 and oi.oSteps>=5 and oi.OrderID in(select OrderID from tbOrderWorkingLogInfo where tbOrderWorkingLogInfo.pAppendTime between @bDate and @eDate and oType=4)  and ol.oWorkType=2 group by oi.StoresID
			) as oo on s.SupplierID=oo.StoresID
			left join 
			--退货
			(select oii.StoresID,isnull(sum(oll.oQuantity*dbo.fun_GetProductCostPrice(oll.ProductsID,oii.oOrderDateTime)),0) CostPrice_t,isnull(sum(oll.oMoney),0) oMoney_t from tbOrderListInfo oll left join tbOrderInfo oii on oii.OrderID=oll.OrderID where oii.oType=2 and oii.oState=0 and oii.oSteps>=5 and oii.OrderID in(select OrderID from tbOrderWorkingLogInfo where tbOrderWorkingLogInfo.pAppendTime between @bDate and @eDate and oType=3) and oll.oWorkType=2 group by oii.StoresID) as ool
			on oo.StoresID=ool.StoresID  
			left join  tbSupplierClassInfo sc on sc.SupplierClassID=s.SupplierClassID) ppp where ppp.oMoney<>0 or ppp.oMoney_t<>0 order by  ppp.SupplierClassID,ppp.SupplierID desc;
	end
...
--全局期初
	...
	isnull(b.bStorage,0) as beginQuantity,isnull(u.uStorage,0) as UpStorage,isnull(u.uInQuantity,0) as UpStorageIn,isnull(u.uOutQuantity,0) as UpStorageOut,
	isnull(t.tInQuantity,0) as StorageIn,isnull(t.tOutQuantity,0) as StorageOut,b.ProductClassID,
	isnull(b.bMoney,0) as beginMoney,
	
	isnull(u.uMoney,0) as UpStorageMoney,isnull(u.uInMoney,0) as UpStorageInMoney,isnull(u.uOutMoney,0) as UpStorageOutMoney,
	...
	isnull(t.tOutQuantity,0)*((isnull(uuu.uuMoney,0)+isnull(uuu.uuInMoney,0)-isnull(uuu.uuOutMoney,0)+isnull(tt.ttInMoney,0))/ (isnull(uuu.uuQuantity,0)+isnull(uuu.uuInQuantity,0)-isnull(uuu.uuOutQuantity,0)+isnull(tt.ttInQuantity,0)))
	 ...
	 from @BeginData as b left join @UpData as u on b.ProductsID=u.ProductsID and b.StorageID=u.StorageID left join 
	 @ThisData as t on t.ProductsID=b.ProductsID and t.StorageID=u.StorageID 
	 left join (
...
ALTER TABLE [dbo].[tbUserOnLineTime] ADD  CONSTRAINT [DF_onlinetime_thismonth]  DEFAULT ((0)) FOR [thismonth]
GO

ALTER TABLE [dbo].[tbUserOnLineTime] ADD  CONSTRAINT [DF_onlinetime_total]  DEFAULT ((0)) FOR [total]
GO

ALTER TABLE [dbo].[tbUserOnLineTime] ADD  CONSTRAINT [DF_onlinetime_lastupdate]  DEFAULT (getdate()) FOR [lastupdate]
GO

ALTER TABLE [dbo].[tbYHsysInfo] ADD  CONSTRAINT [DF_tbYHsysInfo_yAppendTime]  DEFAULT (getdate()) FOR [yAppendTime]
GO
SET IDENTITY_INSERT [tbFeesSubjectClassInfo] ON
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 1,0,N'现金',0,getdate(),0,N'101',0)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 2,0,N'银行',0,getdate(),0,N'102',0)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 3,0,N'库存商品',0,getdate(),0,N'103',0)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 4,0,N'应收账款',0,getdate(),0,N'104',0)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 5,0,N'其他应收款',0,getdate(),0,N'105',0)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 6,0,N'固定资产',0,getdate(),0,N'106',0)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 7,0,N'短期借款',0,getdate(),1,N'201',1)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 8,0,N'应付账款',0,getdate(),1,N'202',1)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 9,0,N'其他应付款',0,getdate(),1,N'203',1)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 10,0,N'应交税金',0,getdate(),1,N'204',1)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 11,0,N'其他应交款',0,getdate(),1,N'205',1)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 12,0,N'实收资本',0,getdate(),1,N'301',2)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 13,0,N'本年利润',0,getdate(),1,N'302',2)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 14,0,N'利润分配',0,getdate(),1,N'303',2)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 17,0,N'商品销售收入',0,getdate(),1,N'401',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 18,0,N'商品销售成本',0,getdate(),0,N'402',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 19,0,N'商品销售税金及附加',0,getdate(),0,N'403',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 20,0,N'经营费用',0,getdate(),0,N'404',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 21,20,N'商场',0,getdate(),0,N'404001',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 22,21,N'促销折让',0,getdate(),0,N'404001001',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 23,21,N'陈列费',0,getdate(),0,N'404001002',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 70,21,N'其它',0,getdate(),0,N'404001011',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 34,33,N'差旅费',0,getdate(),0,N'404002001',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 37,33,N'办公费用',0,getdate(),0,N'404002004',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 38,33,N'业务招待费',0,getdate(),0,N'404002005',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 39,33,N'市内交通',0,getdate(),0,N'404002006',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 40,33,N'赠品贴pop',0,getdate(),0,N'404002007',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 41,33,N'其他',0,getdate(),0,N'404002008',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 42,0,N'管理费用',0,getdate(),0,N'405',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 43,0,N'财务费用',0,getdate(),0,N'406',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 44,43,N'汇款手续费',0,getdate(),0,N'406001',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 46,43,N'利息收支',0,getdate(),0,N'406002',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 60,48,N'请车费用',0,getdate(),0,N'404003001',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 61,48,N'仓储工资',0,getdate(),0,N'404003002',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 71,6,N'扫描仪',0,getdate(),0,N'106001',0)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 73,42,N'社保费',0,getdate(),0,N'405011',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 74,42,N'医保费',0,getdate(),0,N'405012',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 75,0,N'其他收入',0,getdate(),1,N'407',2)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 76,21,N'管理费',0,getdate(),0,N'404001012',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 77,0,N'所得税',0,getdate(),0,N'408',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 78,48,N'市内交通',0,getdate(),0,N'404003009',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 79,6,N'华硕笔记本',0,getdate(),0,N'106002',0)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 80,6,N'打印机',0,getdate(),0,N'106003',0)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 81,6,N'投影机、投影幕',0,getdate(),0,N'106004',0)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 82,38,N'特殊',0,getdate(),0,N'404002005001',0)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 24,21,N'返利',0,getdate(),0,N'404001003',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 25,21,N'配送费',0,getdate(),0,N'404001004',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 26,21,N'店庆',0,getdate(),0,N'404001005',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 27,21,N'合同费用',0,getdate(),0,N'404001006',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 28,21,N'进场费',0,getdate(),0,N'404001007',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 29,21,N'破损买货',0,getdate(),0,N'404001008',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 30,21,N'条码费',0,getdate(),0,N'404001009',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 31,21,N'促销工资',0,getdate(),0,N'404001010',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 33,20,N'业务相关',0,getdate(),0,N'404002',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 35,33,N'业务工资',0,getdate(),0,N'404002002',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 36,33,N'业务提成',0,getdate(),0,N'404002003',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 62,48,N'搬运费',0,getdate(),0,N'404003003',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 63,48,N'采购运费',0,getdate(),0,N'404003004',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 65,48,N'仓库费用',0,getdate(),0,N'404003006',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 67,48,N'工作餐',0,getdate(),0,N'404003008',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 48,20,N'仓储',0,getdate(),0,N'404003',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 49,42,N'办公费用',0,getdate(),0,N'405001',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 50,42,N'电话费',0,getdate(),0,N'405002',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 51,42,N'房租水电',0,getdate(),0,N'405003',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 52,42,N'人员工资',0,getdate(),0,N'405004',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 53,42,N'市内交通',0,getdate(),0,N'405005',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 54,42,N'汽车费用',0,getdate(),0,N'405006',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 55,42,N'维修费',0,getdate(),0,N'405007',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 56,42,N'招待费',0,getdate(),0,N'405008',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 57,42,N'开票税点',0,getdate(),0,N'405009',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 58,42,N'其它',0,getdate(),0,N'405010',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 64,48,N'托运费',0,getdate(),0,N'404003005',3)
INSERT [tbFeesSubjectClassInfo] ([FeesSubjectClassID],[cParentID],[cClassName],[cOrder],[cAppendTime],[cDirection],[cCode],[cType]) VALUES ( 66,48,N'仓库租金',0,getdate(),0,N'404003007',3)
SET IDENTITY_INSERT [tbFeesSubjectClassInfo] OFF
GO
