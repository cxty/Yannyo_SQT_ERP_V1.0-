...
$(document).ready(function () {
    $("#tTree").jstree({
        ...
        $.post(
				"/Services/CAjax.aspx?do=DataClass&doValue=create&DataType=SupplierClass",
				{
        ...
        $.post(
				"/Services/CAjax.aspx?do=DataClass&doValue=rename&DataType=SupplierClass",
				{
        ...
        data.rslt.obj.each(function () {
            $.ajax({
                async: false,
                type: 'POST',
                url: "/Services/CAjax.aspx?do=DataClass&doValue=remove&DataType=SupplierClass",
                data: {
                    "pid": this.pid,
    ...
    $("#btn_delete").click(function () {
        ...
        if (Content != '') {
            jConfirm('是否确认删除选中项？删除后将无法恢复！', '友情提示', function (r) {
                if (r) {
                    $.post("/supplierclass.aspx?Act=del&trNode=" + Content, '', function (data) {
                        if (data == "1") {
                            //刷新页面
                            sObj = -1;
                            getDetails(sObj);
    ...
    $("#add_folder").click(function () {
        ...
        if (dClassName != '') {
            this.PageBox = dialog("添加供货商分类", "iframe:supplierclass_edit.aspx?Act=add&classID=" + dClassID, "450px", "350px", "iframe");
        }
        else {
...
function getDetails(cID) {
    //1.点击节点post
    $.post("/supplierclass.aspx?Act=getNode&classID=" + cID, "", function (data) {
        if (data != '') {
        ...
        jConfirm('是否确认删除选中【' + sName + '】？删除后将无法恢复！', '友情提示', function (r) {
            if (r) {
                if (sObj == '') {
                    ...
                    $.post("/supplierclass.aspx?Act=del&trNode=" + sObj, '', function (data) {
                        if (data == "1") {
                            if ($("#tSel").val() == '') {
                                //刷新页面
                                sObj = -1;
...
function upDate(cID) {
    if (cID) {
        this.PageBox = dialog("修改供货商分类", "iframe:supplierclass_edit.aspx?Act=update&classID=" + cID, "450px", "350px", "iframe");
    }
    else {
