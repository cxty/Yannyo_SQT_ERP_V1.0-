...
$(document).ready(function () {
    $("#tTree").jstree({
        ...
        $.post(
				"/Services/CAjax.aspx?do=DataClass&doValue=create&DataType=Storage",
				{
        ...
        $.post(
				"/Services/CAjax.aspx?do=DataClass&doValue=rename&DataType=Storage",
				{
        ...
        data.rslt.obj.each(function () {
            $.ajax({
                async: false,
                type: 'POST',
                url: "/Services/CAjax.aspx?do=DataClass&doValue=remove&DataType=Storage",
                data: {
                    "pid": this.pid,
    ...
    $("#btn_delete").click(function () {
        ...
        if (Content != '') {
            jConfirm('是否确认删除选中项？删除后将无法恢复！', '友情提示', function (r) {
                if (r) {
                    $.post("/storageclass.aspx?Act=del&trNode=" + Content, '', function (data) {
                        if (data == "1") {
                            //刷新页面
                            sObj = -1;
                            getDetails(sObj);
    ...
    $("#add_folder").click(function () {
        ...
        if (dClassName != '') {
            this.PageBox = dialog("添加仓库分类", "iframe:storageclass_edit.aspx?Act=add&classID=" + dClassID, "450px", "350px", "iframe");
        }
        else {
...
function getDetails(cID) {
    //1.点击节点post
    $.post("/storageclass.aspx?Act=getNode&classID=" + cID, "", function (data) {
        if (data != '') {
        ...
        jConfirm('是否确认删除选中【' + sName + '】？删除后将无法恢复！', '友情提示', function (r) {
            if (r) {
                if (sObj == '') {
                    ...
                    $.post("/storageclass.aspx?Act=del&trNode=" + sObj, '', function (data) {
                        if (data == "1") {
                            if ($("#tSel").val() == '') {
                                //刷新页面
                                sObj = -1;
...
function upDate(cID) {
    if (cID) {
        this.PageBox = dialog("修改仓库分类", "iframe:storageclass_edit.aspx?Act=update&classID=" + cID, "450px", "350px", "iframe");
    }
    else {
